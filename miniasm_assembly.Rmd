...
The trying run failed because of `OUT_OF_MEMORY` (on 27/01/18). Previously I submitted using `--ntasks-per-node=3` and `--mem=16gb`. I think for another try I'll do `--ntasks-per-node=1` and `--mem=100gb` to see if it runs. Also since `racon` needs `.fastq` files, I'll run on the `BJE3652.all.subreads.nolengthlimit.q30.fastq.gz` created by concatenating the files created using `reformat.sh` script from `bbmap` (submitted on 29/01/18).
