...
INPUTS: sequenced reads, their quality scores and information concerning their pairing (`.fastb` + `.qualb` obtained with `PrepareAllPathsInputs.pl`) + ploidy files (only `0=haploid`, `1=diploid`).
...
Potential memory issues considering the number of intermediate and "final" files produced even when just running the assembly on the test data -> a looooot of files will be created in new directories also created by the program. No information if they are necessary for running or just for recovery, if memmory issues, try moving intermediate files produced by the 1st command to freezer and then run the assembly.
