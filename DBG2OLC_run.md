...
Potential issues: (Iqaluk a little bit slow), sounds like cannot read `gz` files, not clear if OK `fastq` for Pacbio (some people got issues with that on some discussion groups).
```
